...
const PasswordInput = ({ label, stateType}) => {
    const [ showEye, setShowEye] = useState( false )
    const [ showPassword, setShowPassword] = useState(false)

    return (
    <div><label>{label}</label>
        <input type={ showPassword ? 'text' : 'password' } onFocus={ () => setShowEye(true) } onBlur={ () => setShowEye(false) } onChange={e => stateType(e.target.value)}></input>
